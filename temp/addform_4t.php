...
if($_SESSION['autoryzacjapracowni']=='razdwatrzybabajagapatrzy'){
  ...
  if($_REQUEST['rodzaj']=='porcelana'){
    ...
    $smarty->display('addform_4i_porcelana_1.tpl');
  }
  elseif( isset($_REQUEST['porcelana']) ){//zrobic case`a i przekazywac zapamietane zmienne
    //$smarty->assign('rodzaj', 'wywolana strona');
    $smarty->display('addform_4i_porcelana_'.$_REQUEST['porcelana'].'.tpl');
  }
  elseif($_REQUEST['rodzaj']=='proteza'){
    //$smarty->assign('rodzaj', $_REQUEST['rodzaj']);
    $smarty->display('addform_4i_proteza_1.tpl');
  }
  elseif( isset($_REQUEST['proteza']) ){
    //$smarty->assign('rodzaj', 'wywolana strona');
    $smarty->display('addform_4i_proteza_'.$_REQUEST['proteza'].'.tpl');
  }
  elseif($_REQUEST['rodzaj']=='niesklasyfikowane'){
    //$smarty->assign('rodzaj', $_REQUEST['rodzaj']);
    $smarty->display('addform_4i_niesklasyfikowane_1.tpl');
  }
  elseif( isset($_REQUEST['niesklasyfikowane']) ){
    //$smarty->assign('rodzaj', 'wywolana strona');
    $smarty->display('addform_4i_niesklasyfikowane_'.$_REQUEST['niesklasyfikowane'].'.tpl');
  }
