...
//sprawdzenie czy pin znajduje sie w bazie pinow i pobranie danych logujacej sie osoby
 $sql="select * from logpin where pin='".MD5($pin)."' ";
...
//pobranie wynikow zapytania do tablicy
 ...
 if(mysql_numrows($sql_result)==1 && $arr['status']=='act') {
    ...
    $_SESSION['panelgorny']='pracpin';
    $_SESSION['strona']='zlecenia_add/addform_1.php';//'temp/tymczasowa.php';

    header("Location: pracownia.php");
 }
 //jezeli status uzytkownika jest 'del'
 elseif(mysql_numrows($sql_result)==1 && $arr['status']<>'act'){
    echo '<br />NIE MO�NA PO��CZY� SI� Z BAZ� DANYCH PONIEWA� TO KONTO U�YTKOWNIKA ZOSTA�O ZABLOKOWANE, W CELU ODBLOKOWANIA DOST�PU SKONTAKTUJ SI� Z ADMINISTRATOREM SYSTEMU';
 }
 //jezeli pin nie wystepuje w bazie
    ...
    if($c==10) {
        ...
        $c=1;//echo ' --------------- '.$c;
        echo '<br />IP ZOSTA� ZABLOKOWANY - ZBYT WIELE NIEUDANYCH PR�B LOGOWANIA, W CELU ODBLOKOWANIA SKONTAKTUJ SI� Z ADMINISTRATOREM SYSTEMU';
    }
    else{
	  ...
	  include('./inc/smarty_path.inc.php');
          echo '<br />liczba nieudanych pr�b logowania: '.$c;
          $smarty->assign('count', $c);
	       $smarty->display('loginpin.tpl');

      }
      else{
          echo '<br />NIE MO�NA PO��CZY� SI� Z BAZ� DANYCH Z TEGO IP - '.$ip.', W CELU ODBLOKOWANIA DOST�PU SKONTAKTUJ SI� Z ADMINISTRATOREM SYSTEMU';
      }	
